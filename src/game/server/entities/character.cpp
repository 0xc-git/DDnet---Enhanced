...
void CCharacter::FireWeapon()
{
	...
	switch(m_Core.m_ActiveWeapon)
	{
		...
		case WEAPON_GUN:
		{
			if ((!m_Jetpack || !m_pPlayer->m_NinjaJetpack) && !m_GrenadeGun)
			{
				int Lifetime;
				if (!m_TuneZone)
		...
		case WEAPON_GRENADE:
		{
			if (m_Core.m_ActiveWeapon == WEAPON_GUN && !m_GrenadeGun)
				break;
		...
		case WEAPON_RIFLE:
		{
			if (m_FreezePlasma)
			{
				new CPlasma(GameWorld(), ProjStartPos, Direction*10.f, true, false, Team(), m_pPlayer->GetCID(), 1.f, 4.f);
			}
			else
			{
				float LaserReach;
	...
	if(!m_ReloadTimer)
	{
		...
		int Delay = FireDelay;
		if (m_Core.m_ActiveWeapon == WEAPON_RIFLE && m_FreezePlasma)
			Delay = 400.f;
		m_ReloadTimer = Delay * Server()->TickSpeed() / 1000;
	}
}
...
void CCharacter::HandleTiles(int Index)
{
	...
	if (m_TileIndex == TILE_REMOVE_NAME || m_TileFIndex == TILE_REMOVE_NAME)
		m_pPlayer->m_HideName = true;

	if (m_TileIndex == TILE_ADD_NAME || m_TileFIndex == TILE_ADD_NAME)
		m_pPlayer->m_HideName = false;

	bool White = m_TileIndex == TILE_COLOR_WHITE || m_TileFIndex == TILE_COLOR_WHITE;
	bool Red = m_TileIndex == TILE_COLOR_RED || m_TileFIndex == TILE_COLOR_RED;
