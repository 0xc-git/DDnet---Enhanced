...
void CGameContext::ConFreezePlasma(IConsole::IResult* pResult, void* pUserData)
{
	CGameContext* pSelf = (CGameContext*)pUserData;
	...
	pChr->m_FreezePlasma = !pChr->m_FreezePlasma;
	if (pChr->m_FreezePlasma)
	{
		pChr->GiveWeapon(WEAPON_RIFLE);
		pSelf->SendChatTarget(Victim, "You got freeze plasma");
	}
	else
