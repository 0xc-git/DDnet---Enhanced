...
void CGameContext::ConGrenadeGun(IConsole::IResult* pResult, void* pUserData)
{
	CGameContext* pSelf = (CGameContext*)pUserData;
	...
	if (pSelf->Server()->GetAuthedState(pResult->m_ClientID) < AUTHED_HELPER)
	{
		pSelf->SendChatTarget(pResult->m_ClientID, "Missing permission");
		return;
	}

	pChr->m_GrenadeGun = !pChr->m_GrenadeGun;
	if (pChr->m_GrenadeGun)
		pSelf->SendChatTarget(pResult->m_ClientID, "You got grenade gun");
	else
		pSelf->SendChatTarget(pResult->m_ClientID, "You lost grenade gun");
}
...
void CGameContext::ConRainbow(IConsole::IResult* pResult, void* pUserData)
{
	...
	if (pChr->m_Rainbow)
		pSelf->SendChatTarget(Victim, "You got rainbow");
	else
		pSelf->SendChatTarget(Victim, "You lost rainbow");
}
...
void CGameContext::ConMember(IConsole::IResult* pResult, void* pUserData)
{
	...
	pSelf->SendChatTarget(Victim, "/grenadegun || For GrenadeGun ");
	pSelf->SendChatTarget(Victim, "/rainbow || For rainbow");
	pSelf->SendChatTarget(Victim, "/freezeplasma || Awesome laser!");
}
