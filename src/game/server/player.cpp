...
void CPlayer::Snap(int SnappingClient)
{
	...
	if(!pClientInfo)
		return;
	//HideName Tile <---------
	StrToInts(&pClientInfo->m_Name0, 4, m_HideName ? " " : Server()->ClientName(m_ClientID));
	//HIDE NAME END HERE <---
	StrToInts(&pClientInfo->m_Clan0, 3, Server()->ClientClan(m_ClientID));
	pClientInfo->m_Country = Server()->ClientCountry(m_ClientID);
	StrToInts(&pClientInfo->m_Skin0, 6, m_TeeInfos.m_SkinName);

	if (m_pCharacter && m_pCharacter->m_Rainbow) // but before that can happen, we will look if he has rainbow and set something diffrent :P custom color ofc on, or we cant set colors xd (1 = true, 0 = false), but this is an integer, so its better to use numbers and not true or false as word
	{
	...
	else if (m_pCharacter && m_pCharacter->m_FakeColor != -1)
	{
		pClientInfo->m_UseCustomColor = 1;
		pClientInfo->m_ColorBody = m_pCharacter->m_FakeColor;
		pClientInfo->m_ColorFeet = m_pCharacter->m_FakeColor;
	}
	else // in this else the tee information is set that the player has chosen in menu
	{
